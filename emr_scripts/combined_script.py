...
spark = SparkSession.builder \
    .appName("validationandtransformation") \
    .getOrCreate()
    # .config("spark.driver.extraClassPath", "/path/to/hadoop-aws.jar:/path/to/aws-java-sdk-bundle.jar") # Commented out for local execution
...
# --- Local Output Paths for Transformed Data (Simulating Curated Zone) ---
local_transformed_output_base_path = "/Users/gyauk/Desktop/DataEngineering/phase_2/Lab4/transformed_data"
local_transformed_vehicles_path = f"{local_transformed_output_base_path}/vehicles/"
local_transformed_locations_path = f"{local_transformed_output_base_path}/locations/"
local_transformed_users_path = f"{local_transformed_output_base_path}/users/"
local_transformed_rentals_path = f"{local_transformed_output_base_path}/rentals/"


print("--- Starting Job 1: All Data Transformation and Curated Data Storage ---")
...
if df_vehicles:
    ...
    # Filter out rows where date conversions failed
    ...
    # Standardize 'license_type' to uppercase
    ...
    print(f"Saving transformed vehicle data to {local_transformed_vehicles_path}")
    df_vehicles_transformed.write.mode("overwrite").parquet(local_transformed_vehicles_path)
...
if df_users:
    ...
    # Validate date conversions (check for nulls after conversion)
    initial_rows = df_users_transformed.count()
    df_users_transformed = df_users_transformed.filter(col("driver_license_expiry").isNotNull()) \
                                               .filter(col("creation_date").isNotNull())
    invalid_date_rows = initial_rows - df_users_transformed.count()
    if invalid_date_rows > 0:
        print(f"WARNING: Dropped {invalid_date_rows} rows due to invalid date formats in driver_license_expiry or creation_date.")
    ...
    print(f"Saving transformed user data to {local_transformed_users_path}")
    df_users_transformed.write.mode("overwrite").parquet(local_transformed_users_path)
...
if df_rentals:
    ...
    # Then filter out nulls resulting from bad casts or original nulls
    ...
    df_rentals_transformed = df_rentals_transformed.filter(col("total_amount").isNotNull()) \
                                                   ...
                                                   .filter(col("rental_start_time") < col("rental_end_time")) # Filter out invalid durations
    ...
    print(f"Saving transformed rental data to {local_transformed_rentals_path}")
    df_rentals_transformed.write.mode("overwrite").parquet(local_transformed_rentals_path)
...
if df_locations:
    ...
    print(f"Saving transformed location data to {local_transformed_locations_path}")
    df_locations_transformed.write.mode("overwrite").parquet(local_transformed_locations_path)
    df_locations_transformed.unpersist() # Free memory after saving

# Stop Spark Session
spark.stop()
print("\nSpark Session stopped. All transformed data saved to local Parquet folders.")
